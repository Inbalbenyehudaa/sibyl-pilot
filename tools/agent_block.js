...
function sibylFieldOn(line) {
  ...
  do {
    ...
    s = s.replace(/^\s+/, '')
         ...
         .replace(/^\d{1,2}\s*[.)]\s*/, '')
         /* §58 — the first Opus 5 run numbered its headings with a MIDDLE DOT
            ("## 1 · failed_checks_banner"), not "1." or "1)". The peel above
            never fired, every heading failed both matchers, and 4 of 13
            fields survived only as inline bold restatements. Same class of
            miss the comment below calls a parser bug in a model-failure
            costume — so the peel learns the separator. */
         .replace(/^\d{1,2}\s*[·:—–]\s+/, '');
  } while (s !== prev);
...
function sibylBoundaryNames(line) {
  ...
  do {
    ...
    s = s.replace(/^\s+/, '')
         ...
         .replace(/^\*{1,3}/, '')
         .replace(/^\d{1,2}\s*[.)]\s*/, '')
         .replace(/^\d{1,2}\s*[·:—–]\s+/, '');   /* §58 — middle-dot numbering */
  } while (s !== prev);
  s = s.replace(/[\*\s]+$/, '');
  if (!s) return null;
...
const WALK_UP_DONE =
  ...
  'every labelled output field, quoting the figures above verbatim. Each label sits bare at ' +
  'the start of its own line — one label per line, never numbered, never merged with another ' +
  'label, never wrapped in a Markdown heading.';
...
const WALK_UP_FINAL =
  ...
  'output field, quoting the figures above verbatim. Each label sits bare at the start of its ' +
  'own line — one label per line, never numbered, never merged with another label, never ' +
  'wrapped in a Markdown heading.';
...
const WALK_UP_STANDS =
  ...
  'full — every labelled output field, quoting its figures verbatim. Each label sits bare at ' +
  'the start of its own line — one label per line, never numbered, never merged with another ' +
  'label, never wrapped in a Markdown heading.';
