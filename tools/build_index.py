...
/* BASELINE PAIR — the configuration that went 5/5 in section 19 was
   ...
   2026-08-08, on the user's instruction: STAGE 2 MOVED TO OPUS 5 — the
   forecast triangulation is the judgment-heavy turn and the user wants a
   reasoning-tier model on it (Sonnet was a cost choice for build/test runs).
   The request shape stays legal unchanged: Opus 5 takes the same
   `thinking: {adaptive, summarized}` + `output_config.effort` pair, rejects
   the same sampling params, and thinking is on by default there anyway. The
   tool loop already echoes the assistant `content` verbatim (thinking blocks
   included), which Opus 5 requires. Same tokenizer family as Opus 4.8;
   MAX_TOKENS_SIBYL 32000 already budgets for thinking. Stage 1 stays Sonnet 5
   (the section-22 lesson was about the reviewer's precision, and Sonnet holds
   it). PENDING: live five-eval re-run — the model rule in PILOT_STATE. */
const MODEL_REVIEWER = 'claude-sonnet-5';     /* stage 1 — one deal reviewer per open deal */
const MODEL_SIBYL = 'claude-opus-5';          /* stage 2 — the judgment-heavy Sibyl turn */
